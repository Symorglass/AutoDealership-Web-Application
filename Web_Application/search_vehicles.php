...
//Conditions
    ...
    if (!empty($VIN) or !empty($vehicle_type) or !empty($manufacturer) or !empty($model_year) or !empty($model_name) or !empty($color) or !empty($min_list_price) or !empty($max_list_price) or ((int)$max_list_price == 0) or !empty($keyword)) {
        if (!empty($VIN)) {
            $query = $query . " AND ( vi.VIN = '$VIN')";
        }
        if (!empty($vehicle_type)) {
            $query = $query . "  AND (vi.vehicle_type = '$vehicle_type'  )";
        }
        if (!empty($manufacturer)) {
            $query = $query . "  AND (vi.manufacturer = '$manufacturer' )";
        }
        if (!empty($model_year)) {
            $query = $query . "  AND (vi.model_year = '$model_year' )";
        }
        if (!empty($model_name)) {
            $query = $query . "  AND (vi.model_name = '$model_name' )";
        }
        if (!empty($color)) {
            $query = $query . "  AND (vi.color LIKE '%$color%')";
        }
        if (!empty($min_list_price)) {
            $query = $query . "  AND (vi.list_price > '$min_list_price')";
        }
        if (!empty($max_list_price) or ((int)$max_list_price == 0)) {
            $query = $query . "  AND (vi.list_price < '$max_list_price' )";
        }
        if (!empty($keyword) && $keyword_is_case_sensitive == 'sensitive') {
            $query = $query . "  AND (vi.manufacturer LIKE BINARY '%$keyword%') OR (vi.model_year LIKE BINARY '%$keyword%' ) OR (vi.model_name LIKE BINARY '%$keyword%' ) OR (vi.description LIKE BINARY '%$keyword%')";
        }
        if (!empty($keyword) && $keyword_is_case_sensitive == 'insensitive') {
            $query = $query . "  AND (vi.manufacturer LIKE '%$keyword%') OR (vi.model_year LIKE '%$keyword%' ) OR (vi.model_name LIKE '%$keyword%' ) OR (vi.description LIKE '%$keyword%')";
        }
    }
...
<title>Vehicles Search - Public</title>
</head>
        ...
        <div class="center_left">
            ...
            <div class="features">
                ...
                <div class="profile_section">
                    ...
                    <form name="searchform" action="search_vehicles.php" method="POST">
                        <table>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td><input type="text" name="min_list_price"/></td>
                            </tr>
                            <tr>
                                <td class="item_label">Max List Price</td>
                                <td><input type="text" name="max_list_price"/></td>
                            </tr>
                            <tr>
